...
function parseRow(name, value) {
    ...
    const normalizeAmount = (amount) => amount.replace('$', '').replace(',', '');
    ...
    if (normalizedName === 'date') {
        ...
        const date = (0, date_fns_1.parse)(value, 'MMMM d, yyyyh:mm a', new Date());
        if ((0, date_fns_1.isValid)(date)) {
            return { date: date.toISOString() };
        }
        const date2 = (0, date_fns_1.parse)(value, 'MMMM d, yyyy', new Date());
        if ((0, date_fns_1.isValid)(date2)) {
 ...
 */
function getTransactionDescription(element) {
    ...
    const transactionHeader = document.evaluate(transactionHeaderExp, element, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    const description = transactionHeader?.textContent;
    return description;
}
function processTransactionDetails(element) {
    let rows = [];
    ...
    if (!rowData.account && (rowData.to || rowData.from)) {
        rowData = { ...rowData, account: (rowData.total ?? 0) < 0 ? rowData.from : rowData.to };
    }
...
function parsedTransactionsToCsv(parsed) {
    ...
    const replacer = (_key, value) => (value === null || value === undefined ? '' : value); // specify how you want to handle null values here
    const headers = Object.keys(items.reduce((acc, item) => {
...
async function exportTransactions() {
    ...
    const result = Array.from(roleElement).map(processTransactionDetails);
